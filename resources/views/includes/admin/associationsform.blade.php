<div class="form-group required @if($errors->has('faculty_id_id')) has-error @endif">
    {!! Form::label('faculty_id','Faculteit',['class' => 'control-label']) !!}
    {!! Form::select('faculty_id', $faculties, null, ['class' => 'form-control', 'value' => Input::old('faculty_id')]) !!}
    @if ($errors->has('faculty_id')) <p class="help-block">{{ $errors->first('faculty_id') }}</p> @endif
</div>
...
<div class="form-group required @if($errors->has('body')) has-error @endif">
    {!! Form::label('body','Verhaal',['class' => 'control-label']) !!}
